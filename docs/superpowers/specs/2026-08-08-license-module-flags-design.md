...
Correção após mapear os arquivos reais: o mobile **não** reaproveita `LicenseFullViewModel` de graça — staff carrega a lista de condomínios via `LicenseViewModel` (`GET api/access/licenses`) e morador carrega o próprio perfil via `ResidentProfileViewModel` (`GET api/resident/me`), dois DTOs de cliente diferentes do usado pelo portal. `EnabledModules` precisa ser adicionado aos três (`LicenseFullViewModel`, `LicenseViewModel`, `ResidentProfileViewModel`), todos em `Condotify.Contracts`, mais um enum espelho `Condotify.Models.LicenseModuleEnum` (mesmos bits do enum do servidor, já que o mobile não referencia `CondotifyAPI.Domain`). `MobileNavigation.For(...)` (`Condotify.Mobile.Core`) ganha uma sobrecarga que recebe o bitmask e filtra os itens de navegação inferior; `More.razor` (que concentra a maior parte dos atalhos de módulo, fora da navegação inferior) filtra os próprios links da mesma forma. `MobileAppState` passa a guardar o bitmask do morador (staff já tem via `SelectedLicense.EnabledModules`).
