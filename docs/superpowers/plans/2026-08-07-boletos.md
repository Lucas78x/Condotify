...
public static class BoletoPageMatcher
{
    ...
    internal static MatchResult MatchByUnitText(string pageText, IReadOnlyCollection<UnitCandidate> units)
    {
        ...
        var matchedUnits = units
            .Where(unit => ContainsUnitReference(normalized, unit))
            .Select(unit => unit.UnitId)
            .Distinct()
            .ToList();

        return matchedUnits.Count == 1
            ? new MatchResult(matchedUnits[0], BoletoMatchMethodEnum.UnitText)
            : new MatchResult(null, BoletoMatchMethodEnum.Unmatched);
    }
    ...
    internal static bool ContainsUnitReference(string normalizedText, UnitCandidate unit)
    {
        var unitKey = Normalize(unit.UnitNumber);
        if (string.IsNullOrWhiteSpace(unitKey)) return false;

        var unitPattern = $@"\b(apto|apartamento|unidade|ap)\.?\s*{Regex.Escape(unitKey)}\b";
        if (!Regex.IsMatch(normalizedText, unitPattern)) return false;

        var blockKey = Normalize(unit.BlockName);
        return string.IsNullOrWhiteSpace(blockKey) || normalizedText.Contains(blockKey, StringComparison.Ordinal);
    }
