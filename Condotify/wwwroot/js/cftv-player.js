window.condotifyCftv = (() => {
    ...
    async function start(elementId, url) {
        ...
        peer.ontrack = event => { video.srcObject = event.streams[0] ?? new MediaStream([event.track]); };
        try {
            ...
            if (!response.ok) throw new Error(`O gateway de vídeo respondeu HTTP ${response.status}.`);
            if (!answer.trim()) throw new Error("O gateway retornou uma negociação vazia.");
            await peer.setRemoteDescription({ type: "answer", sdp: answer }); await waitForConnection(peer); video.muted = true; await video.play();
        } catch (error) {
    ...
    async function stop(elementId) {
        ...
        session.controller.abort(); session.peer.close(); session.video.pause(); session.video.srcObject = null; sessions.delete(elementId);
    }
    return { start, stop };
})();
